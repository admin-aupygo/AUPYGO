...
(function () {
  ...
  window.createAdminStaffGroup = async function () {
    ...
    try {
      var created = await supabaseClient.from('conversations').insert({
        created_by: currentUser.id, type: 'group', title: title
      }).select().single();
      ...
      setGroupChatHeader(title, cid, true);
    } catch (e) {
  ...
  if (typeof _origOpenCreate === 'function') {
    window.openCreateGroupModal = function () {
      ...
      if (isStaff()) { showToast('Seul l Admin peut créer des groupes.', 'error'); return; }
  ...
  function setGroupChatHeader(title, gid, canDelete) {
    var header = document.getElementById('chatHeader');
    if (!header) return;
    var safeTitle = title || 'Groupe';
    header.removeAttribute('data-i18n');
    header.innerHTML = '';
    var wrap = document.createElement('div');
    wrap.style.cssText = 'display:flex;align-items:center;justify-content:space-between;gap:10px;width:100%;flex-wrap:wrap';
    var span = document.createElement('span');
    span.style.fontWeight = '700';
    span.textContent = safeTitle;
    wrap.appendChild(span);
    if (canDelete && typeof isAdmin === 'function' && isAdmin()) {
      var btn = document.createElement('button');
      btn.type = 'button';
      btn.className = 'btn btn-secondary';
      btn.style.cssText = 'font-size:12px;padding:6px 12px;background:#fef2f2;color:#b91c1c;border:1px solid #fecaca';
      btn.textContent = '🗑️ Supprimer le groupe';
      btn.onclick = function () { window.deleteStaffGroup(gid, safeTitle); };
      wrap.appendChild(btn);
    }
    header.appendChild(wrap);
  }

  window.deleteStaffGroup = async function (gid, title) {
    if (!(typeof isAdmin === 'function' && isAdmin())) {
      showToast('Seul l Admin peut supprimer un groupe.', 'error');
      return;
    }
    if (!gid) return;
    ...
    try {
      ...
      if (staffGroupIdCache === gid) staffGroupIdCache = null;
      window.activeConversation = null;
      var box = document.getElementById('chatMessages');
      if (box) box.innerHTML = '<div class="chat-placeholder"><p>Groupe supprimé.</p></div>';
      var header = document.getElementById('chatHeader');
      if (header) header.textContent = 'Sélectionne une conversation à gauche';
      showToast('Groupe supprimé pour tous les membres.', 'success');
      await applyStaffMessagesUI();
    } catch (e) {
      showToast('Erreur: ' + (e.message || e), 'error');
    }
  ...
  async function applyStaffMessagesUI() {
    ...
    document.querySelectorAll(
      ...
      if (/^amis$/i.test(t) || /^équipe$/i.test(t) || /^equipe$/i.test(t)) el.textContent = 'Staff';
    });
    ...
    if (friendsList) {
      if (typeof isAdmin === 'function' && isAdmin()) {
        ...
        friendsList.innerHTML = admin ? buildMemberRow(admin) : '<p class="conv-empty">Admin introuvable.</p>';
      } else {
    ...
    document.querySelectorAll('#messages button').forEach(function (btn) {
  ...
  window.openStaffGroup = async function () {
    ...
    markRead(gid);
    setGroupChatHeader(STAFF_GROUP_TITLE, gid, typeof isAdmin === 'function' && isAdmin());
    applyStaffMessagesUI();
  };
  ...
  window.openStaffAnyGroup = async function (gid, title) {
    ...
    await renderStaffChat(gid, true);
    markRead(gid);
    setGroupChatHeader(title || 'Groupe', gid, true);
    applyStaffMessagesUI();
  };
