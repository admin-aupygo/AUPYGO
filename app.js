...
function openCreateEventModal(visibility) {
  ...
  if (visibility === 'admin') {
    ...
    if (special) { special.classList.add('selected'); selectEventEmoji(special); }
  } else {
    if (picks[0]) { picks[0].classList.add('selected'); selectEventEmoji(picks[0]); }
  }
        ...
        <div class="event" data-event-id="${ev.id}">
          ...
          <div class="event-body">
            ...
            ` : `
              ...
              ${isJoined
                ? `<button class="btn btn-secondary" style="margin-top:10px" disabled>✅ Tu participes</button>`
                : full
                  ? `<button class="btn btn-secondary" style="margin-top:10px" disabled>Complet</button>`
                  : `<button class="btn btn-primary event-join" style="margin-top:10px" onclick="joinRealEvent('${ev.id}')">Participer</button>`
              }
            `}
          </div>
        </div>`;
