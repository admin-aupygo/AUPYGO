...
async function loadDmHistory(friendId) {
  ...
  if (error) {
    console.error('loadDmHistory:', error);
    box.innerHTML = '';
  ...
  data.forEach(m => appendBubble(m.body, m.sender_id === currentUser.id));
  box.scrollTop = box.scrollHeight;
}
  ...
  messagesChannel = supabaseClient
    ...
    .on('postgres_changes',
      ...
      (payload) => {
        ...
        if (activeConversation && activeConversation.type === 'dm' && activeConversation.id === msg.sender_id) {
          appendBubble(msg.body, false);
        } else {
